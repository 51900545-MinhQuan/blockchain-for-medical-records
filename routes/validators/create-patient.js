...
const CreatePatientValidator = [
  check('fullname')
    .exists().withMessage('Họ và tên là trường bắt buộc.')
  ...
  check('birthday')
    .exists().withMessage('Ngày sinh là trường bắt buộc.')
  ...
  check('phone')
    .exists().withMessage('Số điện thoại là trường bắt buộc.')
    .notEmpty().withMessage('Vui lòng nhập số điện thoại.')
    .isLength({ min: 10, max: 11 }).withMessage('Số điện thoại phải có từ 10 đến 11 chữ số.'),

  check('gender')
    .exists().withMessage('Giới tính là trường bắt buộc.')
